...
public class MusicAlbumActivity extends AppCompatActivity implements View.OnClickListener {

    public final static String TAG = "OnlineMusicActivity";

    MyImageView iv_icon;
    ImageView nextIv, playIv, lastIv, playStyle,love_music,iv_back;
    TextView singerTv, songTv, currentProTv, totalProTv;
    RecyclerView musicRv;
    LrcView lrcView;
    View inc,inc_online,inc_love;
    //数据源
    List<OnlineMusicBean> mDatas = new ArrayList<>();
    ...
    protected void onResume() {
        ...
        mDatas.clear();
        findAlbumMusic(DiscoverFragment.type);
    ...
    private class MyBroadCastRecever extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            final Notification_Controller notificationController = new Notification_Controller(MusicAlbumActivity.this);
    ...
    public void findAlbumMusic(String str){

        BmobQuery<OnlineMusicBean> bmobQuery = new BmobQuery<>();
        bmobQuery.addWhereEqualTo("album",str);
        bmobQuery.findObjects(new FindListener<OnlineMusicBean>() {
            @Override
            public void done(List<OnlineMusicBean> list, BmobException e) {
                Log.i("ping",""+list.size());
                if (e == null) {
                    ...
                    //创建适配器对象
                    adapter = new OnlineMusicAdapter(MusicAlbumActivity.this, mDatas);
                    musicRv.setAdapter(adapter);
                    //设置布局管理器
                    LinearLayoutManager layoutManager = new LinearLayoutManager(MusicAlbumActivity.this, LinearLayoutManager.VERTICAL, false);
